    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-bold text-2xl text-white leading-tight flex items-center gap-2">
                <span>✏️</span> Editar Área: {{ $area->nombre }}
            </h2>
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl backdrop-blur">
            <form action="{{ route('areas.update', $area) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre', $area->nombre) }}" required class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">
                </div>

                <!-- Encargado Responsable -->
                <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Encargado Responsable</label>
                    <select name="encargado_id" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">
                        ...
                        @foreach ($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" {{ old('encargado_id', $area->encargado_id) == $usuario->id ? 'selected' : '' }}>
                                {{ $usuario->name }} ({{ $usuario->email }})
                            </option>
                        @endforeach
                    </select>
                    <p class="text-xs text-slate-500 mt-1">Al cambiar el encargado, las nuevas responsabilidades de inventario quedarán registradas a su nombre.</p>
                </div>

                <!-- Descripción -->
                <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Descripción</label>
                    <textarea name="descripcion" rows="3" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">{{ old('descripcion', $area->descripcion) }}</textarea>
                </div>

                <!-- Estado -->
                <div class="flex items-center">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="estado" value="1" class="sr-only peer" {{ old('estado', $area->estado) ? 'checked' : '' }}>
