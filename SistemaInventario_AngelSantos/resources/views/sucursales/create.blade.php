        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl backdrop-blur">
            <form action="{{ route('sucursales.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="empresa_id" required class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">
                        @foreach ($empresas as $empresa)
                            <option value="{{ $empresa->id }}" {{ old('empresa_id', request('empresa_id')) == $empresa->id ? 'selected' : '' }}>
                                {{ $empresa->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" required placeholder="Ej. Sucursal Centro, Sede Norte, Bodega Logística" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">
                </div>

                <!-- Teléfono -->
                <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Teléfono de la Sucursal</label>
                    <input type="text" name="telefono" value="{{ old('telefono') }}" placeholder="Ej. +504 2234-9900" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">
                </div>

                <!-- Dirección -->
                <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Dirección de la Sucursal</label>
                    <textarea name="direccion" rows="3" placeholder="Ubicación exacta..." class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">{{ old('direccion') }}</textarea>
                </div>
