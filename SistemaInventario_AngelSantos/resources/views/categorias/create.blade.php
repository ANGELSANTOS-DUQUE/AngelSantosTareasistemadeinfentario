        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl backdrop-blur">
            <form action="{{ route('categorias.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" value="{{ old('nombre') }}" required placeholder="Ej. Lácteos, Herramientas, Electrónica" class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">
                </div>

                <!-- Descripción -->
                <div>
                    <label class="block text-sm font-semibold text-slate-300 mb-2">Descripción (Opcional)</label>
                    <textarea name="descripcion" rows="3" placeholder="Detalles de la categoría..." class="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none">{{ old('descripcion') }}</textarea>
                </div>
